---
output-dir: docs 
title: "Gyps Vulture Survival SUpplemental Materials" 
author: "Brian W. Rolek" 
email: "brianrolek at gmail dot com" 
date: "25 Sept 2026" 
format: html 
df-print: kable 
editor: visual 
...
![1. Vultures photographed by Corinne Kendall](vultures.jpg)
...
```{r}
#| include: false 
knitr::read_chunk('R/03-analyses.R')
```
...
We compared the predictive accuracy of models using WAIC. We didn't evaluate the code here because it has a long runtime. Instead, we load the results file to inspect the results. The waic.func() is a custom function that uses Nimble's calculateWAIC() to estimate WAIC values. This function is located in the functions.R file.
...
#| eval: true 
#| warning: false 
#| tbl-cap: "Table S1. Model comparison using WAIC."
```
...
```{r}
#| include: false 
knitr::read_chunk('R/04-analysis-sensitivity.R')
```
...
#| eval: true 
#| warning: false
#| tbl-cap: "Table S2. Survival estimates when substituting observation state 2 with either 3, 4, or 5 to evaluate the sensitivity of models to misclassification. Survival here is presented as monthly survival."
```

## 5. Postprocessing survival
...
```{r}
#| label: traceplots global 
#| eval: true 
#| warning: false 
#| fig-cap: "Figure S1: Traceplots (left) and density plots (right) of parameters from the global model. In density plots the black line depicts posterior density while the red line depicts prior density." 
#| fig-subcap: 
#| - "" 
#| - "" 
#| - "" 
...
```{r}
#| label: traceplots reduced  
#| eval: true  
#| warning: false  
#| fig-cap: "Figure S2: Traceplots (left) and density plots (right) of parameters from the reduced model. In density plots the black line depicts posterior density while red line depicts prior density." 
#| fig-subcap: 
#| - "" 
#| - "" 
#| - ""
...
#| eval: true 
#| warning: false 
#| tbl-cap: "Table S3. Parameter estimates from the null, global, and reduced models."
```
...
```{r}
#| label: tag failure age 
#| eval: true 
#| warning: false 
#| fig-cap: "Figure S3. Predicted tag failure in response to age of transmitter." 
#| fig-subcap: 
...
```{r}
#| label: tag failure year 
#| eval: true 
#| warning: false 
#| fig-cap: "Figure S4. Predicted tag failure in response to year." 
#| fig-subcap: 
#| - "(A) Yearly probabilities (above)" 
#| - "(B) Monthly probabilities (above)" 
#| layout-ncol: 1 
...
#| eval: true 
#| warning: false 
#| fig-cap: "Figure S5. Predicted survival (yearly) by study period (A) and combined (B)."
```
...
#| eval: true 
#| warning: false 
#| tbl-cap: "Table S4. Yearly survival estimates. Survival during early, late, and combined periods."
```

### 5.5 Survival differences by age class

```{r}
#| label: survival differences 
#| eval: true 
#| warning: false 
#| tbl-cap: "Table S5. Compare survival differences by age class."
#| layout-ncol: 1
options(kableExtra.auto_format = FALSE)
```

## 6. Matrix population model

```{r}
#| include: false 
knitr::read_chunk('R/06-matrix-pop-model.R')
```
